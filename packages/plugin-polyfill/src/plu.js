...
const getExtensions = (options, optionExtensionName, optionPath) => {
  ...
  if (isDefined(options[optionExtensionName])) {
    if (isString(options[optionExtensionName])) {
      ...
      assert(
        isArray(extensions),
        `${optionPath}.${optionExtensionName} should be a string or array of strings (${extensions})`,
      )
  ...
  if (!isMetaTag && isString(tag)) {
    ...
    if (isDefined(tag.glob) || isDefined(tag.globPath) || isDefined(tag.globFlatten)) {
      ...
      const globAssetPaths = globAssets.map(globAsset =>
        slash(path.join(tag.path, flatten ? path.basename(globAsset) : globAsset)),
      )
      assert(
        globAssetPaths.length > 0,
        `${optionPath}.${optionName} object glob found no files (${tag.path} ${assetGlob} ${globPath})`,
      )
      tagObjects = []
...
const getValidatedMainOptions = (options, optionPath, defaultOptions = {}) => {
  const {append, prependExternals, publicPath, usePublicPath, addPublicPath, hash, useHash, addHash, ...otherOptions} =
    options
...
const getTagPath = (tagObject, options, webpackPublicPath, compilationHash) => {
  ...
  Object.keys(tagObject)
    .filter(key => isDefined(tagObject[key]))
    .forEach(key => {
